    ...
    <Card className="flex flex-col h-[480px]">
      ...
      <div className="relative overflow-y-auto custom-scroll -m-2 -mt-1 p-2 space-y-2 pr-3">
        <AnimatePresence initial={false}>
          {last100.map(a => <AlertRow key={a.id + '_' + a.timestamp} alert={a} />)}
        </AnimatePresence>
...
function AlertRow({alert}){
  return (
    <motion.div
      ...
      exit={{opacity:0, y:-6}}
      className={`group rounded-md border border-white/10 bg-neutral-800/40 px-3 py-2 text-xs backdrop-blur relative ${Date.now()-alert._receivedAt < 1500 ? 'animate-flash':''}`}
    >
      ...
      <p className="mt-1 text-neutral-200 leading-snug">{alert.description}</p>
    </motion.div>
  );
}
